---
outline: deep
description: Estimates gas to initiate a deposit MNT transaction on an L1, which executes a transaction on an L2.
---

# estimateDepositMNTGas

Estimates gas to initiate a [deposit MNT transaction](https://github.com/mantlenetworkio/mantle-v2/blob/v1.0.0-alpha.1/packages/contracts-bedrock/contracts/L1/L1StandardBridge.sol#L197) on an L1, which executes a transaction on an L2.
...
const gas = await publicClientL1.estimateDepositMNTGas({
  account,
  request: {
    amount: parseEther('1'),
  },
...
```ts
const gas = await client.estimateDepositMNTGas({
