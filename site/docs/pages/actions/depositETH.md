---
outline: deep
description: Initiates a ETH deposit transaction on an L1, which executes a transaction on an L2.
---

# depositETH

Initiates a ETH deposit transaction on an L1, which executes a transaction on an L2.

Internally performs a contract write to the [`depositETH` function](https://github.com/mantlenetworkio/mantle-v2/blob/v1.0.0-alpha.1/packages/contracts-bedrock/contracts/L1/L1StandardBridge.sol#L162) on the [Mantle L1StandardBridge contract](https://github.com/mantlenetworkio/mantle-v2/blob/v1.0.0-alpha.1/packages/contracts-bedrock/contracts/L1/L1StandardBridge.sol).
...
```ts
const hash = await client.depositETH({
...
Value in wei sent with this ETH deposit transaction on the L2. Debited from the caller's L2 balance.
